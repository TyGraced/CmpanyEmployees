...
namespace CompnyEmployees.Utility
{
    public class CompanyLinks : ICompanyLinks
    {
        ...
        public LinkResponse TryGenerateLinks(IEnumerable<CompanyDto> companiesDto, string fields,
        ...
        private List<Entity> ShapeData(IEnumerable<CompanyDto> companiesDto, string fields) =>
            _dataShaper.ShapeData(companiesDto, fields).Select(e => e.Entity).ToList();
        ...
        private LinkResponse ReturnLinkedCompanies(IEnumerable<CompanyDto> companiesDto,
            string fields, HttpContext httpContext, List<Entity> shapedCompanies)
        {
            var companyDtoList = companiesDto.ToList();

            for (var index = 0; index < companyDtoList.Count(); index++)
            {
                var companyLinks = CreateLinksForCompany(httpContext, companyDtoList[index].Id,
                    fields);
        ...
        private List<Link> CreateLinksForCompany(HttpContext httpContext, Guid id, string fields = "")
        {
            ...
            {
                new Link(_linkGenerator.GetUriByAction(httpContext, "GetCompany",
        ...
        private LinkCollectionWrapper<Entity> CreateLinksForCompanies(HttpContext httpContext,
            LinkCollectionWrapper<Entity> companiesWrapper)
        {
            companiesWrapper.Links.Add(new Link(_linkGenerator.GetUriByAction(httpContext,
